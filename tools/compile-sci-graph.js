// Compile data/science_curriculum_v2.json into the live compiled graph + seed migration.
// Mirrors tools/compile-math-graph.js for the Science domain.
// Usage: node tools/compile-sci-graph.js
const fs = require('fs');
...
const v2 = R('data/science_curriculum_v2.json');
...
const oldLangIds = new Set(mg.nodes.filter(n => n.domain === 'Science').map(n => n.id));
const isLang = id => uIds.has(id) || oldLangIds.has(id);

// ---- 1. coords: reuse existing, inherit for new ----
const oldById = new Map(mg.nodes.filter(n => n.domain === 'Science').map(n => [n.id, n]));
const coord = new Map();
...
const langNodes = uNodes.map(n => ({
  id: n.id, title: n.title, domain: 'Science',
  ...
  legacy_name: n.title, legacy_subject: 'Science', source: n.provenance || 'v2',
  game_skill: n.game_skill || null, stage_id: n.stage_id || null, atomic: n.atomic !== false
}));
const others = mg.nodes.filter(n => n.domain !== 'Science');
...
let sql = `-- English/Science curriculum graph v2 seed (generated by tools/compile-sci-graph.js — do not hand-edit)\n`;
...
for (const n of langNodes) {
  ...
  sql += `'${esc(n.id)}','${esc(n.title)}','Science','${esc(n.path_type)}','${esc(n.stage)}','${esc(gradeBand(n.grade_band))}',${n.primary_path},'${esc(n.cluster)}','${esc(n.description)}','${esc(n.description)}',${jsonArr(n.mastery_criteria)},'${esc(v)}'::jsonb,'${esc(n.title)}','Science','${esc(n.source)}')\n`;
...
sql += `DELETE FROM curriculum_edges WHERE from_node IN (SELECT id FROM curriculum_nodes WHERE domain='Science') OR to_node IN (SELECT id FROM curriculum_nodes WHERE domain='Science');\n`;
let ec = 0;
for (const e of langIncident) {
  const idc = 'SCV2' + String(++ec).padStart(5, '0');
...
sql += `\nCOMMIT;\n`;
fs.writeFileSync(path.join(BACKEND, 'supabase/migrations/zzzzzz_science_graph_v2_seed.sql'), sql);
...
console.log(`seed: supabase/migrations/zzzzzz_science_graph_v2_seed.sql (${langIncident.length} edges)`);
