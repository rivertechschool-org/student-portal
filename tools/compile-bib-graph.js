// Compile data/bible_curriculum_v2.json into the live compiled graph + seed migration.
// Mirrors tools/compile-math-graph.js for the Science domain.
// Usage: node tools/compile-bib-graph.js
const fs = require('fs');
...
const v2 = R('data/bible_curriculum_v2.json');
...
const oldLangIds = new Set(mg.nodes.filter(n => n.domain === 'Bible').map(n => n.id));
const isLang = id => uIds.has(id) || oldLangIds.has(id);

// ---- 1. coords: reuse existing, inherit for new ----
const oldById = new Map(mg.nodes.filter(n => n.domain === 'Bible').map(n => [n.id, n]));
const coord = new Map();
...
const langNodes = uNodes.map(n => ({
  id: n.id, title: n.title, domain: 'Bible',
  ...
  legacy_name: n.title, legacy_subject: 'Bible', source: n.provenance || 'v2',
  game_skill: n.game_skill || null, stage_id: n.stage_id || null, atomic: n.atomic !== false
}));
const others = mg.nodes.filter(n => n.domain !== 'Bible');
...
let sql = `-- English/Science curriculum graph v2 seed (generated by tools/compile-bib-graph.js — do not hand-edit)\n`;
...
for (const n of langNodes) {
  ...
  sql += `'${esc(n.id)}','${esc(n.title)}','Bible','${esc(n.path_type)}','${esc(n.stage)}','${esc(gradeBand(n.grade_band))}',${n.primary_path},'${esc(n.cluster)}','${esc(n.description)}','${esc(n.description)}',${jsonArr(n.mastery_criteria)},'${esc(v)}'::jsonb,'${esc(n.title)}','Bible','${esc(n.source)}')\n`;
...
sql += `DELETE FROM curriculum_edges WHERE from_node IN (SELECT id FROM curriculum_nodes WHERE domain='Bible') OR to_node IN (SELECT id FROM curriculum_nodes WHERE domain='Bible');\n`;
let ec = 0;
for (const e of langIncident) {
  const idc = 'BIV2' + String(++ec).padStart(5, '0');
...
sql += `\nCOMMIT;\n`;
fs.writeFileSync(path.join(BACKEND, 'supabase/migrations/zzzzzz_bible_graph_v2_seed.sql'), sql);
...
console.log(`seed: supabase/migrations/zzzzzz_bible_graph_v2_seed.sql (${langIncident.length} edges)`);
