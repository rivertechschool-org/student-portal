// Compile data/english_curriculum_v2.json into the live compiled graph + seed migration.
// Mirrors tools/compile-math-graph.js for the Language domain.
// Usage: node tools/compile-lang-graph.js
const fs = require('fs');
...
const v2 = R('data/english_curriculum_v2.json');
...
// old Language node ids = existing Language-domain nodes in master_graph
const oldLangIds = new Set(mg.nodes.filter(n => n.domain === 'Language').map(n => n.id));
const isLang = id => uIds.has(id) || oldLangIds.has(id);

// ---- 1. coords: reuse existing, inherit for new ----
const oldById = new Map(mg.nodes.filter(n => n.domain === 'Language').map(n => [n.id, n]));
const coord = new Map();
...
// ---- 3. new Language nodes in master_graph schema ----
const langNodes = uNodes.map(n => ({
  id: n.id, title: n.title, domain: 'Language',
  ...
  legacy_name: n.title, legacy_subject: 'Language', source: n.provenance || 'v2',
  game_skill: n.game_skill || null, stage_id: n.stage_id || null, atomic: n.atomic !== false
}));
const others = mg.nodes.filter(n => n.domain !== 'Language');
...
// keep every edge NOT touching Language, and cross_domain edges touching Language (endpoints still exist — no merges deleted nodes)
for (const e of eg.edges) {
  ...
  // drop old Language prerequisite_hard/leads_to (regenerated below)
}
...
let sql = `-- English/Language curriculum graph v2 seed (generated by tools/compile-lang-graph.js — do not hand-edit)\n`;
sql += `-- ${langNodes.length} Language nodes, ${langIncident.length} Language-incident edges. Apply via SQL editor or supabase db push.\nBEGIN;\n\n`;
sql += `-- 1. Upsert all Language nodes (existing kept, new/split added)\n`;
for (const n of langNodes) {
  ...
  sql += `'${esc(n.id)}','${esc(n.title)}','Language','${esc(n.path_type)}','${esc(n.stage)}','${esc(gradeBand(n.grade_band))}',${n.primary_path},'${esc(n.cluster)}','${esc(n.description)}','${esc(n.description)}',${jsonArr(n.mastery_criteria)},'${esc(v)}'::jsonb,'${esc(n.title)}','Language','${esc(n.source)}')\n`;
...
sql += `\n-- 2. Replace all Language-incident edges (nodes upserted above; cross-domain re-inserted below)\n`;
sql += `DELETE FROM curriculum_edges WHERE from_node IN (SELECT id FROM curriculum_nodes WHERE domain='Language') OR to_node IN (SELECT id FROM curriculum_nodes WHERE domain='Language');\n`;
let ec = 0;
for (const e of langIncident) {
  const idc = 'ENV2' + String(++ec).padStart(5, '0');
...
sql += `\nCOMMIT;\n`;
fs.writeFileSync(path.join(BACKEND, 'supabase/migrations/zzzzzz_english_graph_v2_seed.sql'), sql);

console.log('=== LANG COMPILE DONE ===');
console.log(`master_graph: ${mg.nodes.length} nodes (${langNodes.length} Language + ${others.length} other)`);
...
console.log(`seed: supabase/migrations/zzzzzz_english_graph_v2_seed.sql (${langIncident.length} edges)`);
