// Compile data/physical_curriculum_v2.json into the live compiled graph + seed migration.
// Mirrors tools/compile-math-graph.js for the Science domain.
// Usage: node tools/compile-phys-graph.js
const fs = require('fs');
...
const v2 = R('data/physical_curriculum_v2.json');
...
const oldLangIds = new Set(mg.nodes.filter(n => n.domain === 'Physical').map(n => n.id));
const isLang = id => uIds.has(id) || oldLangIds.has(id);

// ---- 1. coords: reuse existing, inherit for new ----
const oldById = new Map(mg.nodes.filter(n => n.domain === 'Physical').map(n => [n.id, n]));
const coord = new Map();
...
const langNodes = uNodes.map(n => ({
  id: n.id, title: n.title, domain: 'Physical',
  ...
  legacy_name: n.title, legacy_subject: 'Physical', source: n.provenance || 'v2',
  game_skill: n.game_skill || null, stage_id: n.stage_id || null, atomic: n.atomic !== false
}));
const others = mg.nodes.filter(n => n.domain !== 'Physical');
...
let sql = `-- Physical curriculum graph v2 seed (generated by tools/compile-phys-graph.js — do not hand-edit)\n`;
sql += `-- ${langNodes.length} Physical nodes, ${langIncident.length} Physical-incident edges. Apply via SQL editor or supabase db push.\nBEGIN;\n\n`;
// Merged umbrella nodes (P4/P5/P6/P7/P9 split into P-3xx skills). The upsert pattern never
// deletes rows, so drop them explicitly; curriculum_edges FKs cascade.
const droppedIds = (v2.merges || []).map(m => m.base_id).filter(id => !uIds.has(id));
if (droppedIds.length) {
  sql += `-- 0. Remove umbrella nodes that were split into granular skills (edges cascade)\n`;
  sql += `DELETE FROM curriculum_nodes WHERE domain='Physical' AND id IN (${droppedIds.map(id => `'${esc(id)}'`).join(',')});\n\n`;
}
sql += `-- 1. Upsert all Physical nodes (existing kept, new/split added)\n`;
for (const n of langNodes) {
  ...
  sql += `'${esc(n.id)}','${esc(n.title)}','Physical','${esc(n.path_type)}','${esc(n.stage)}','${esc(gradeBand(n.grade_band))}',${n.primary_path},'${esc(n.cluster)}','${esc(n.description)}','${esc(n.description)}',${jsonArr(n.mastery_criteria)},'${esc(v)}'::jsonb,'${esc(n.title)}','Physical','${esc(n.source)}')\n`;
...
sql += `DELETE FROM curriculum_edges WHERE from_node IN (SELECT id FROM curriculum_nodes WHERE domain='Physical') OR to_node IN (SELECT id FROM curriculum_nodes WHERE domain='Physical');\n`;
let ec = 0;
for (const e of langIncident) {
  const idc = 'PHV2' + String(++ec).padStart(5, '0');
...
sql += `\nCOMMIT;\n`;
fs.writeFileSync(path.join(BACKEND, 'supabase/migrations/zzzzzz_physical_graph_v2_seed.sql'), sql);

// ---- 6. shared/pe-data.js (node data + assess metadata for training-log.html / pe-assessment.html) ----
const peNodes = {};
for (const n of uNodes) peNodes[n.id] = { id: n.id, title: n.title, cluster: n.cluster, stage: n.stage, stage_id: n.stage_id, grade_band: n.grade_band, game_skill: n.game_skill, mastery_criteria: n.mastery_criteria, hard_prereqs: n.hard_prereqs || [], soft_deps: n.soft_deps || [], assess: n.assess };
const skillToId = {}; for (const n of uNodes) skillToId[n.game_skill] = n.id;
const peOut = '// PE Training Log data — generated from data/physical_curriculum_v2.json by tools/compile-phys-graph.js. Do not hand-edit.\n'
  + '// Benchmarks are simplified grade-band standards (FitnessGram-inspired, sex-neutral). [meets, exceeds] per band.\n'
  + 'window.PE_DATA = ' + JSON.stringify({ nodes: peNodes, skillToId, clusters: ['Motor Foundations', 'Fitness Measures', 'Sport & Team Skills', 'Health & Fitness Knowledge', 'Active Habits'] }, null, 1) + ';\n';
fs.writeFileSync(path.join(ROOT, 'shared/pe-data.js'), peOut);

console.log('=== PHYS COMPILE DONE ===');
console.log(`shared/pe-data.js: ${uNodes.length} nodes`);
...
console.log(`seed: supabase/migrations/zzzzzz_physical_graph_v2_seed.sql (${langIncident.length} edges)`);
