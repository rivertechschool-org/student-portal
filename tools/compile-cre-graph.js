// Compile data/creative_curriculum_v2.json into the live compiled graph + seed migration.
// Mirrors tools/compile-math-graph.js for the Science domain.
// Usage: node tools/compile-cre-graph.js
const fs = require('fs');
...
const v2 = R('data/creative_curriculum_v2.json');
...
const oldLangIds = new Set(mg.nodes.filter(n => n.domain === 'Creative').map(n => n.id));
const isLang = id => uIds.has(id) || oldLangIds.has(id);

// ---- 1. coords: reuse existing, inherit for new ----
const oldById = new Map(mg.nodes.filter(n => n.domain === 'Creative').map(n => [n.id, n]));
const coord = new Map();
...
const langNodes = uNodes.map(n => ({
  id: n.id, title: n.title, domain: 'Creative',
  ...
  legacy_name: n.title, legacy_subject: 'Creative', source: n.provenance || 'v2',
  game_skill: n.game_skill || null, stage_id: n.stage_id || null, atomic: n.atomic !== false
}));
const others = mg.nodes.filter(n => n.domain !== 'Creative');
...
let sql = `-- English/Science curriculum graph v2 seed (generated by tools/compile-cre-graph.js — do not hand-edit)\n`;
...
for (const n of langNodes) {
  ...
  sql += `'${esc(n.id)}','${esc(n.title)}','Creative','${esc(n.path_type)}','${esc(n.stage)}','${esc(gradeBand(n.grade_band))}',${n.primary_path},'${esc(n.cluster)}','${esc(n.description)}','${esc(n.description)}',${jsonArr(n.mastery_criteria)},'${esc(v)}'::jsonb,'${esc(n.title)}','Creative','${esc(n.source)}')\n`;
...
sql += `DELETE FROM curriculum_edges WHERE from_node IN (SELECT id FROM curriculum_nodes WHERE domain='Creative') OR to_node IN (SELECT id FROM curriculum_nodes WHERE domain='Creative');\n`;
let ec = 0;
for (const e of langIncident) {
  const idc = 'CRV2' + String(++ec).padStart(5, '0');
...
sql += `\nCOMMIT;\n`;
fs.writeFileSync(path.join(BACKEND, 'supabase/migrations/zzzzzz_creative_graph_v2_seed.sql'), sql);
...
console.log(`seed: supabase/migrations/zzzzzz_creative_graph_v2_seed.sql (${langIncident.length} edges)`);
